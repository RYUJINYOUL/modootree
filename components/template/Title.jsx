...
export default function Title({ username, uid }) {
  ...
  const handleTitleSave = async (newTitle) => {
    ...
    try {
      ...
      const docRef = doc(db, 'users', finalUid, 'info', 'titleSettings');
  ...
  useEffect(() => {
    const loadTitle = async () => {
      ...
      try {
        // Firebase 연결 상태 체크
        if (!navigator.onLine) {
          console.warn('오프라인 상태: 제목을 불러올 수 없습니다.');
          return;
        }
  ...
  const saveStyleSettings = async (newSettings) => {
    ...
    try {
      ...
      await setDoc(doc(db, 'users', finalUid, 'settings', 'title'), newSettings, { merge: true });
  ...
  useEffect(() => {
    const loadStyleSettings = async () => {
      ...
      try {
        ...
        const docRef = doc(db, 'users', finalUid, 'settings', 'title');
        const docSnap = await getDoc(docRef);
        if (docSnap.exists()) {
      ...
      >
        {isEditingTitle ? (
          <input
            ...
            className="text-center bg-transparent border-b border-white/30 focus:border-white/60 outline-none px-2 py-1 w-full"
