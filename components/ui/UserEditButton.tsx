...
export default function UserEditButton({ username, ownerUid, userEmail }: UserEditButtonProps) {
  ...
  // 구독 상태 확인
  useEffect(() => {
    if (!ownerUid || !currentUser?.email) return;
      ...
      try {
        ...
        const unsubscribe = onSnapshot(subscribersRef, (doc) => {
          if (doc.exists()) {
            ...
            setIsSubscribed(
              Object.values(subscribers).some(
                (sub: any) => sub.email === currentUser.email
              )
            );
          }
        });
  ...
  const handleSubscription = async () => {
    ...
    try {
      ...
      const currentSubscribers = docSnap.exists() ? docSnap.data().users || {} : {};
      
      // 이미 구독 중인지 확인
      const isAlreadySubscribed = Object.values(currentSubscribers).some(
        (sub: any) => sub.email === subscribeEmail
      );
      ...
      // 2. 구독자의 구독 목록에도 추가
      if (currentUser?.uid) {
        const userSubscriptionsRef = doc(db, 'users', currentUser.uid, 'settings', 'subscriptions');
        const userSubscriptionsDoc = await getDoc(userSubscriptionsRef);
        const currentSubscriptions = userSubscriptionsDoc.exists() ? userSubscriptionsDoc.data().subscribedPages || {} : {};
