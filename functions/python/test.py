...
def get_embedding(text: str) -> Optional[List[float]]:                              # 7. 임베딩
    try:
...
def classify_query(query: str) -> Tuple[SearchCategory, str]:                      # 8. 스키마 카테고리 대입
    q = query.lower()
    keywords = {
...
def fetch_api_data(source: str, query: str) -> Dict:                                  # 9. 네이버, 구글 데이타 받기
...
def filter_search_results(raw_results: List[Dict]) -> List[Dict]:               # 10. 네이버, 구글 데이터 병합
    cleaned = []
...
def fetch_image_with_cache(name: str, category: SearchCategory, source_url: str = "") -> str:     # 11. 캐쉬이미지 없을 경우 네이버에서 이미지 찾기
...
def generate_prompt(category: SearchCategory, query: str, results: List[Dict]) -> str:     # 12. LLM에게 최종 추천 목록 생성
...
# (QdrantManager의 store_search는 answer_summary를 받도록 수정해야 함을 가정)
# QdrantManager.store_search(..., recommendations=recommendations, sources=sources_data, answer_summary=answer_summary)
...
def stream_search():
    ...
    def generate():
        ...
        try:
            ...
            if qdrant_manager:
                ...
                if query_vector:
                    ...
                    if similar_results and similar_results[0]["score"] > 0.82:
                        ...
                        cached_data = qdrant_manager.get_full_cache_data(similar_results[0]['id'])

                        if cached_data:
                            use_cached_results = True
                            
                            # ✅ 수정 1: 캐시 히트 시 최종 데이터 할당
                            recommendations = cached_data.get("recommendations", [])
                            answer_summary = cached_data.get("query", "") # 💡 "query" 대신 "answer_summary" 사용
                            sources = cached_data.get("sources", [])
                            
                            # cleaned 변수에 캐시 출처를 임시 대입 (최종 반환 시 sources를 만들기 위함)
                            # Qdrant에 전체 검색 결과(cleaned)가 저장되지 않았다면, 이 부분이 필요할 수 있습니다.
                            # 여기서는 sources를 사용하겠습니다.
                            cleaned = sources
                            
                            # 카테고리가 캐시 데이터에 있다면 업데이트
            ...
            # 8. Qdrant 저장 (캐시 미스일 때만)
            if qdrant_manager and not use_cached_results:
                try:
                    # 💡 LLM 호출 후 생성된 answer_summary를 저장
                    sources_data = [
                        {
                            "title": r.get("title", ""),
                    ...
                    qdrant_manager.store_search(
                        ...
                        answer_summary=answer_summary # 💡 추가된 최종 요약
                    )
            ...
            # 최종 sources: 캐시 히트 시 'sources' 변수, 캐시 미스 시 'cleaned' 변수를 기반으로 함.
            # cleaned 변수는 캐시 히트 시 'sources'로 대체되었으므로, cleaned를 사용해도 무방.
            final_sources = [
                {
                    "title": r.get("title", ""),
                    "snippet": r.get("snippet", "")[:100],
