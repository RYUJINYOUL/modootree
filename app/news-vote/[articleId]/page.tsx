// app/news-vote/[articleId]/page.tsx
'use client';

import { useState, useEffect, useCallback } from 'react';
import { doc, getDoc, updateDoc, collection, query, orderBy, onSnapshot, DocumentData, QueryDocumentSnapshot } from 'firebase/firestore'; // collection, query, orderBy, onSnapshot 임포트
import { db } from '@/lib/firebase';
// import Image from 'next/image'; // 제거
import { notFound } from 'next/navigation'; // Not found 페이지를 위해 next/navigation 임포트
...
import { Avatar, AvatarImage, AvatarFallback } from '@/components/ui/avatar'; // Avatar 컴포넌트 임포트
import { Textarea } from '@/components/ui/textarea'; // Textarea 컴포넌트 임포트
...
interface Article {
  ...
  youtube_link?: string; // 유튜브 기능 재활성화 시 필요
}
...
export default function ArticlePage({ params }: ArticlePageProps) {
  ...
  const [newCommentText, setNewCommentText] = useState(''); // 새 댓글 텍스트
  const [commentLoading, setCommentLoading] = useState(false); // 댓글 제출 로딩 상태
  ...
  useEffect(() => {
    const fetchArticle = async () => {
      ...
      try {
        ...
        const data = { id: articleSnap.id, ...articleSnap.data() } as Article; // 타입 단언 추가
        setNewsItem(data);

        // 조회수 증가 (클라이언트 측에서 단순 증가, 서버에서 중복 방지 로직 필요)
    ...
    const unsubscribe = onSnapshot(commentsQuery, (snapshot) => {
      const fetchedComments: Comment[] = snapshot.docs.map(doc => ({
        id: doc.id,
        ...doc.data(),
        createdAt: doc.data().createdAt // Firestore Timestamp 객체 그대로 유지
      })) as Comment[];
  ...
  if (!newsItem) {
    return null; // notFound()가 처리하므로 여기는 도달하지 않음
  }
    ...
    <main className="min-h-screen bg-gradient-to-b from-slate-950 via-blue-950 to-cyan-900 text-white/90 relative overflow-hidden">
      <div className="container mx-auto px-4 py-10 relative z-10 max-w-2xl">
        ...
        <div className="bg-white/10 backdrop-blur-sm rounded-xl p-6 border border-blue-500/20 shadow-lg mb-8">
          ...
          {/* 공유 및 리스트 이동 버튼 (상단으로 이동) */}
          <div className="flex justify-end gap-2 mb-6"> {/* 오른쪽 정렬 및 여백 추가 */}
            <button
              onClick={() => {
