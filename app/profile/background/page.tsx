...
export default function ProfileBackgroundSettings() {
  ...
  useEffect(() => {
    ...
    const fetchCurrentBackground = async () => {
      try {
        const settingsDocRef = doc(db, 'users', currentUser.uid, 'settings', 'profileBackground');
                        ...
                        onChange={async (e) => {
                          ...
                          if (file) {
                            try {
                              ...
                              const storageRef = ref(storage, `profile-backgrounds/bg_${Date.now()}`);
